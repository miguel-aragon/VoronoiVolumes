/*
   ...
   COMPILE AS:
       ...
       gcc -o voronoi_volume_weight voronoi_volume_weight.c io_files.c -lm
...
int main(int argc, char **argv)
{
  ...
  double half_pix;
  double *x_nuclei, *y_nuclei, *z_nuclei, *w_nuclei;
  ...
  int    n_cell; 
  //--- Filenames
  char   *file_inp, *file_wei, *file_out;
  FILE   *fp_inp, *fp_wei, *fp_out;
  char   buffer[255];
  ...
  if (argc != 11) {
    ...
    printf("\n");
    printf(" Usage:\n");
    printf("     ./voronoi_volume_weight  file_nuclei file_weight sizeG sizeX sizeY sizeY offX offY offZ file_out \n");
    printf("\n");
    printf("------------------------------------------ \n");
    exit(0); }
  ...
  w_nuclei = (double *) malloc(sizeof(double) * n_wei * 27);
  w_arr    = (double *) malloc(sizeof(double) * n_wei);
  for (i=0 ; i<n_wei ; i++){            //--- DTFE format
    fread(&tempD,sizeof(double),1,fp_wei);
    w_arr[i] = tempD;
  }
  fclose(fp_wei);

  printf("Replicate weights...\n");
  cont = 0;
  for (i=0 ; i<=2 ; i++){
    for (j=0 ; j<=2 ; j++){
      for (k=0 ; k<=2 ; k++){
  ...
  for (i=0 ; i<sizeX ; i++){  
	...
	for(w=0; w<n_nuclei_periodic; w++) {
	  ...
	  tempD = (pow(xg-x_nuclei[w],2) + pow(yg-y_nuclei[w],2) + pow(zg-z_nuclei[w],2)) / w_nuclei[w];
	...
	//--- Recover original unweighted distance
	//min_dista *= w_nuclei[w]; //-- WRONG, CHECK NON-PERIODIC CODE FOR FIX
